...
namespace Axiom.GeoShape
{
	...
	public class Node3D
	{
		...
		private const string X_Formula = "X";
		private const string Y_Formula = "Y";
		private const string Z_Formula = "Z";
		private const string RotX_Formula = "RotX";
		...
		/// </summary>
		public string XFormula { get => _parameters[X_Formula].Formula; set => _parameters[X_Formula].Formula = value; }

		/// <summary>
		/// Formula della posizione Y
		/// </summary>
		public string YFormula { get => _parameters[Y_Formula].Formula; set => _parameters[Y_Formula].Formula = value; }

		/// <summary>
		/// Formula della posizione Z
		/// </summary>
		public string ZFormula { get => _parameters[Z_Formula].Formula; set => _parameters[Z_Formula].Formula = value; }

		/// <summary>
		/// Formula della rotazione attorno a X (intesa in gradi)
		/// </summary>
		public string RotXFormula { get => _parameters[RotX_Formula].Formula; set => _parameters[RotX_Formula].Formula = value; }

		/// <summary>
		/// Formula della rotazione attorno a Y (intesa in gradi)
		/// </summary>
		public string RotYFormula { get => _parameters[RotY_Formula].Formula; set => _parameters[RotY_Formula].Formula = value; }

		/// <summary>
		/// Formula della rotazione attorno a Z (intesa in gradi)
		/// </summary>
		public string RotZFormula { get => _parameters[RotZ_Formula].Formula; set => _parameters[RotZ_Formula].Formula = value; }
		...
		/// </summary>
		public List<Variable> SecondaryVariables { get; set; }
		...
		public Node3D()
		{
			...
			_parameters = new()
			{
				{ X_Formula, new Parameter(X_Formula, true, "", 0.0) },
				{ Y_Formula, new Parameter(Y_Formula, true, "", 0.0) },
				{ Z_Formula, new Parameter(Z_Formula, true, "", 0.0) },
				{ RotX_Formula, new Parameter(RotX_Formula, true, "", 0.0) },
				{ RotY_Formula, new Parameter(RotY_Formula, true, "", 0.0) },
				{ RotZ_Formula, new Parameter(RotZ_Formula, true, "", 0.0) }
			};
		...
		public void AddNode(Node3D node)
		{
			if (string.IsNullOrEmpty(node.Id))
			{
				...
				{
					node.Id = id.ToString();
					bContinue = Nodes.ContainsKey(node.Id);
					id++;
				}
			}
			node.Path = PathId;
		...
		public virtual bool Update(Dictionary<string, Variable> variables, out string errorDescription)
		{
			...
			if (Delegates.DelegateEvaluator != null)
			{
				...
				foreach (var item in _parameters)
				{
					...
					{
						if (item.Key == X_Formula) X = value;
						if (item.Key == Y_Formula) Y = value;
						if (item.Key == Z_Formula) Z = value;
						if (item.Key == RotX_Formula) xDegAngle = value;
						if (item.Key == RotY_Formula) yDegAngle = value;
						if (item.Key == RotZ_Formula) zDegAngle = value;
					}
				}
